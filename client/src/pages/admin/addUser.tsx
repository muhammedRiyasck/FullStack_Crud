...
export default function AddUserForm() {
  ...
  const handleSubmit = async(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    try {
      ...
      e.preventDefault()
      let Status = isValid(email, password, name);
      setErrors(Status);
  
      if (!Status.email && !Status.password && !Status.name) { 
  
        await axios.post('/admin/addUser',{name,email,password})
    ...
    <div className="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8">
      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm border border-black rounded-lg p-10 shadow-2xl">
        <div className="sm:mx-auto sm:w-full sm:max-w-sm">
          <h2 className="mt-10 mb-9 text-center text-2xl/9 font-bold tracking-tight text-gray-900">Add User Account</h2>
        </div>
